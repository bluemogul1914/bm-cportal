...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add_comment_sd' && $sd_connected) {
        ...
        if (!empty($comment)) {
            ...
            if (isset($result['error'])) {
                $error_msg = 'Failed to add reply: ' . $result['error'];
            } else {
                $success_msg = 'Reply added.';
            }
        }
        ...
        if (!empty($comment)) {
            try {
                ...
                $pdo->prepare("INSERT INTO ticket_comments (ticket_id, user_id, comment, is_internal, created_at) VALUES (?, ?, ?, false, NOW())")->execute([intval($ticket_id), $user_id, $comment]);
                $pdo->prepare("UPDATE tickets SET updated_at = NOW() WHERE id = ? AND client_id = ?")->execute([intval($ticket_id), $client_id]);
                $pdo->prepare("INSERT INTO activity_log (user_id, action, entity_type, entity_id, details, ip_address) VALUES (?, ?, ?, ?, ?, ?)")->execute([$user_id, 'comment_added', 'ticket', $ticket_id, 'Reply on ticket #' . $ticket_id, $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0']);
...
if ($is_sd_source && $sd_connected) {
    ...
    if (!isset($result['error'])) {
        $all = $result['data'] ?? $result['tickets'] ?? $result ?? [];
        if (is_array($all)) {
            foreach ($all as $t) {
                if ((string)($t['ticketId'] ?? $t['id'] ?? '') === (string)$ticket_id) {
                    $ticket = $t;
                    $api_ticket = true;
                    break;
                }
    ...
    if ($api_ticket) {
        $rr = itarian_sd_api('getticketreplies', ['ticketId' => intval($ticket_id)]);
        if (!isset($rr['error'])) {
            $comments = $rr['data'] ?? $rr['replies'] ?? [];
...
if (!$api_ticket) {
    try {
        ...
        $stmt->execute([intval($ticket_id), $client_id]);
...
<head>
    ...
    <title>Ticket #<?php echo htmlspecialchars($tid); ?> - Blue Mogul</title>
                ...
                <div class="flex items-center gap-3 ml-8 mt-1">
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium <?php
                        echo match(true) {
                            ...
                            in_array($status, ['closed', 'resolved']) => 'bg-gray-100 text-gray-700',
                            default => 'bg-yellow-100 text-yellow-700'
                        };
                    ?>" data-testid="status-badge"><?php echo ucfirst(str_replace('_', ' ', $status)); ?></span>
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium <?php
                        echo match(true) {
                            in_array($priority, ['high', 'urgent', 'critical']) => 'bg-red-100 text-red-700',
                            in_array($priority, ['medium', 'normal']) => 'bg-yellow-100 text-yellow-700',
            ...
            <div class="max-w-4xl">
                ...
                <?php if (!$is_closed): ?>
                    <div class="bg-white rounded-lg border border-gray-200 mt-6">
                        ...
                        <form method="POST" action="ticket-detail.php?id=<?php echo urlencode($ticket_id); ?><?php echo $api_ticket ? '&source=sd' : ''; ?>" class="p-6">
                            <?= csrf_field() ?>
                            <input type="hidden" name="action" value="<?php echo $api_ticket ? 'add_comment_sd' : 'add_comment'; ?>">
