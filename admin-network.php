...
if ($client_id) {
    ...
    $stmt = $pdo->prepare("SELECT id, client_id, hostname, device_type, manufacturer, model, serial_number, ip_address, mac_address, os_name, os_version, cpu, ram_gb, disk_gb, status, notes, last_seen, created_at, source, external_id FROM network_devices WHERE client_id = ? ORDER BY device_type ASC, hostname ASC");
    $stmt->execute([$client_id]);
