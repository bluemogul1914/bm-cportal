 ...
 *
 * Usage at top of every dealer page:
 *   require_once __DIR__ . '/../includes/dealer-header.php';
 *   dealer_auth();           // redirects if not dealer session
 *   $dealer = dealer_me();   // returns current dealer row
 *
...
function dealer_me() {
    // Simple cache in session to avoid repeated DB hits per request
    if (!empty($_SESSION['dealer_cache'])) return $_SESSION['dealer_cache'];
    $pdo = get_db();
    $stmt = $pdo->prepare(
        'SELECT id, dealer_code, full_name, email, company, tier,
                activations_mtd, status
         FROM dealers WHERE id = ?'
    );
    $stmt->execute([$_SESSION['dealer_id']]);
...
$_initials = strtoupper(substr($_dealer['full_name'], 0, 1) . (strpos($_dealer['full_name'], ' ') !== false ? substr(strrchr($_dealer['full_name'], ' '), 1, 1) : ''));
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .btn-primary:hover{background:#1648887;}
